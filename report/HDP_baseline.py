...
def hdp(docs, voca, gamma, alpha, beta, epochs=1):
    ...
    for z in range(epochs):
        ...
        for j, x_i in enumerate(x_ji):
            ...
            for i, w in enumerate(x_i):
                doc_j, topic_idx, n_jtw, n_kv, m_k = sample_t(j, i, w, doc_j, topic_idx, n_jtw, n_kv, 
                                                              m_k, gamma, alpha, beta, V)
        ...
        for j in range(D):
            ...
            for tbl in doc_j['t_j']:
                doc_j, topic_idx, n_kv, m_k = sample_k(j, tbl, doc_j, topic_idx, n_jtw, 
                                                       n_kv, m_k, beta, V, gamma)
...
def sample_k(j, tbl,  doc_j, topic_idx, n_jtw, n_kv, m_k, beta, V, gamma):
    ...
    if tbl != 0:
        ...
        post_pvals_k = posterior_k(j, doc_j, tbl, topic_idx, n_jtw, n_kv, m_k, V, beta, gamma)
...
def posterior_k(j, doc_j, tbl, topic_idx, n_jtw, n_kv, m_k, V, beta, gamma):
